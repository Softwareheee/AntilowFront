...
const emptyProduct = { nombre: '', categoria: 'periferico', descripcion: '', precio: '', stock: '', imagen_url: '', caracteristicas: '' }
...
function Inventario({ isAdmin = false }) {
	...
	const submitProduct = async (event) => {
		...
		setError('')
		try {
			const payload = { ...form, precio: Number(form.precio), stock: Number(form.stock), caracteristicas: form.caracteristicas.split(',').map((item) => item.trim()).filter(Boolean) }
	...
	const startEdit = (product) => {
		...
		setForm({ ...product, caracteristicas: (product.caracteristicas || []).join(', ') })
		setMessage('')
		setError('')
	}
